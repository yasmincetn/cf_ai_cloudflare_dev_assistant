              ...
              <div key={message.id} className="space-y-2">
                ...
                {message.parts
                  ...
                  .map((part, i) => {
                    ...
                    if (parsed && parsed.issue_type) {
                      let stepsTitle = "Steps:";
                      if (parsed.issue_type === "product_recommendation") {
                        stepsTitle = "Recommendation:";
                      } else if (parsed.issue_type === "explanation") {
                        stepsTitle = "Next steps:";
                      }

                      const hasCause =
                        parsed.likely_cause && parsed.likely_cause !== "null";

                      const hasSteps =
                        Array.isArray(parsed.recommended_steps) &&
                        parsed.recommended_steps.length > 0;

                      const issueIcon =
                        parsed.issue_type === "troubleshooting"
                          ? "🛠"
                          : parsed.issue_type === "product_recommendation"
                            ? "💡"
                            : parsed.issue_type === "explanation"
                              ? "📘"
                              : "🤖";
                      ...
                      return (
                        <div key={i} className="flex justify-start">
                          <div className="max-w-[85%] p-4 rounded-2xl bg-kumo-base border border-kumo-line space-y-3 shadow-sm">
                            <div className={`text-sm font-semibold ${issueColor}`}>
                              {issueIcon} {
                                parsed.issue_type === "troubleshooting"
                                ? "Troubleshooting"
                                : parsed.issue_type === "product_recommendation"
                            ...
                            <div className="text-xs text-kumo-inactive">
                              📦 Product:
                              <span className="ml-1 px-2 py-1 bg-sky-100 text-sky-700 rounded">
                                {parsed.related_product === "Multiple" ? "Multiple Products" : parsed.related_product}
                              </span>
                            </div>

                            <div className="text-sm">
                              <b>Summary:</b> {parsed.summary}
                            </div>

                            {hasCause && (
                              <div className="text-sm text-yellow-600">
                                <b>Cause:</b> {parsed.likely_cause}
                              </div>
                            )}
